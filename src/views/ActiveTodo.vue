<template>
  <section class="main">
    <input
      ...
      v-model="toggleAll"
    />
    <label for="toggle-all" v-show="baseData.length > 0">
      反转所有事项的完成情况
    </label>
    <ul class="list">
      <li
...
export default {
  ...
  computed: {
    todoData() {
      return this.baseData.filter((item) => item.completed === false);
    },
    toggleAll: {
      get() {
        if (this.baseData.length === 0) return false; //如果当前没有事项就不显示
        //箭头根据当下未完成事项的数量点亮
        return this.left === 0;
      },
      set(value) {
        //根据当前计算属性的值渲染所有事项状态
        this.baseData.forEach((item) => (item.completed = value));
      },
    },
  },
  watch: {},
